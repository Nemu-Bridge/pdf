...
class Terminal {
  ...
  header(): void {
    console.log(
      `${colors.gray(symbols.pointer)} build started at ${this.format_time()}\n`,
    );
  }
  ...
  start_step(label: string): void {
    ...
    process.stdout.write(
      `  ${colors.gray(symbols.corner)} ${colors.cyan(this.initial_frame)} ${label}...`,
    );

    this.spinner_timer = setInterval(() => {
      this.current_frame =
        (this.current_frame + 1) % this.spinner_frames.length;
      const frame = this.spinner_frames[this.current_frame] as string;
      process.stdout.write(
        `${clear_line}  ${colors.gray(symbols.corner)} ${colors.cyan(frame)} ${label}...`,
      );
    }, 80);
  }
  ...
  succeed_step(msg: string): void {
    ...
    console.log(
      `${clear_line}  ${colors.gray(symbols.corner)} ${colors.green(symbols.check)} ${msg} ${time}`,
    );
  }
  ...
  fail_step(msg: string): void {
    ...
    console.log(
      `${clear_line}  ${colors.gray(symbols.corner)} ${colors.red(symbols.cross)} ${colors.red(msg)}`,
    );
  }
  ...
  finish(success: boolean): void {
    ...
    console.log(
      `  ${colors.gray("[")}${status}${colors.gray("]")} build completed in ${time}`,
    );
    console.log("");
  }
}
...
async function main(): Promise<void> {
  ...
  const esm_result = await Bun.build({
    ...
    format: "esm",
    naming: "[name].mjs",
    external: ["pdfkit", "svg-to-pdfkit", "@mathjax/src", "@mathjax/mathjax-termes-font", "@mathjax/mathjax-newcm-font", "@mathjax/mathjax-modern-font", "@mathjax/mathjax-pagella-font", "@mathjax/mathjax-stix2-font", "@mathjax/mathjax-fira-font"],
  });
  ...
  const types_result =
    await $`tsc --declaration --emitDeclarationOnly --outDir dist --module esnext --target es2022 --moduleResolution bundler --skipLibCheck src/index.ts`.nothrow();
  ...
  ui.start_step("bundling fonts");
  await $`mkdir -p dist/fonts`.nothrow();
  await $`cp src/fonts/geist-mono.ttf src/fonts/geist.ttf src/fonts/inter.ttf src/fonts/nunito-sans.ttf src/fonts/roboto.ttf src/fonts/source-serif-4.ttf src/fonts/OFL.txt dist/fonts/`.nothrow();
  if (!existsSync("dist/fonts/inter.ttf")) {
    ui.fail_step("fonts were not copied into dist/fonts");
    process.exit(1);
  }
  ui.succeed_step("fonts bundled");

  ui.start_step("vendoring harfbuzz");
  await $`cp node_modules/harfbuzzjs/dist/harfbuzz-subset.wasm dist/harfbuzz-subset.wasm`.nothrow();
  if (!existsSync("dist/harfbuzz-subset.wasm")) {
    ui.fail_step("harfbuzz-subset.wasm missing; is harfbuzzjs installed?");
    process.exit(1);
  }
  ui.succeed_step("harfbuzz vendored");
