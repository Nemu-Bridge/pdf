...
export const color_css = (color: ColorLike | undefined): string | undefined => {
  if (color === undefined) return undefined;
  if (typeof color === "string") return color;
  return color.to_hex();
};

export const pdf_rgb = (color: ColorLike): [number, number, number] => {
  const c = parse_color(color);
  return [c.r * 255, c.g * 255, c.b * 255];
};

export const line_gap = (style: StyleProperties, font_size: number): number =>
  style.line_height ? (style.line_height - 1) * font_size : 2;

export const parse_color = (color: ColorLike): ColorValue => {
