...
import { weight_value } from "./instancer";
import {
...
import { pdf_rgb, color_css, line_gap } from "./style";
...
export const resolve_element_style = (
  ...
  const base =
    classname && theme ? theme.merge_classname(style ?? {}, classname) : style ?? {};
  return theme?.apply_to_style(base, element_type) ?? base;
};
...
export abstract class BaseElement {
  ...
  protected _resolve_style(
    ...
    return resolve_element_style(style, classname, element_type, this._theme);
  }
  ...
  text(options: CreateTextOptions): TextElement {
    const el = make_text(options, this._theme, this._doc_parse_markdown);
    this._children.push(el);
    return el;
  }

  rect(options: CreateRectOptions): RectElement {
    const el = make_rect(options, this._theme, this._doc_parse_markdown);
    this._children.push(el);
    return el;
  }

  image(options: CreateImageOptions): ImageElement {
    const el = make_image(options, this._theme, this._doc_parse_markdown);
    this._children.push(el);
    return el;
  }

  create_container(options?: CreateContainerOptions): ContainerElement {
    const el = make_container(options, this._theme, this._doc_parse_markdown);
    this._children.push(el);
    return el;
  }
}
    ...
    if (this.resolved_style.background_color) {
      try {
        pdoc
          ...
          .fill(pdf_rgb(this.resolved_style.background_color));
      } catch {}
    }
  ...
  render(pdoc: any): void {
    ...
    for (let ri = 0; ri < this.rows.length; ri++) {
      ...
      if (is_header && this.options.header_style?.background_color) {
        try {
          pdoc
            ...
            .fill(pdf_rgb(this.options.header_style.background_color));
        } catch {}
      }
      ...
      for (let ci = 0; ci < this.col_widths.length; ci++) {
        ...
        if (border_width > 0) {
          try {
            pdoc
              ...
              .strokeColor(pdf_rgb(border_color))
        ...
        const inner_w = Math.max(0, col_w - padding * 2);
        const cell_family = resolve_font_family(merged, this._document!);
        const cell_weight = weight_value(merged.font_weight);
        const cell_base =
          cell_weight == null
            ? cell_family
            : weighted_font(pdoc, this._document!, cell_family, cell_weight);
        const segments =
          cell && typeof cell === "object" ? cell.segments : undefined;
        if (segments) {
          render_segments(
            pdoc,
            segments,
            ...
            { x: col_x + padding, y: row_y + padding },
            color_css(merged.color),
            (_b, bo, it) =>
              seg_variant(
                pdoc,
                this._document!,
...
const wire = <T extends BaseElement>(
  el: T,
  theme: Theme | null,
  parse_markdown: boolean,
): T => {
  el._theme = theme;
  el._doc_parse_markdown = parse_markdown;
  return el;
};

export const make_text = (
  options: CreateTextOptions,
  theme: Theme | null,
  parse_markdown: boolean,
): TextElement =>
  wire(
    new TextElement(
      options,
      resolve_element_style(options.style, options.classname, "text", theme),
      parse_markdown,
    ),
    theme,
    parse_markdown,
...
export const make_rect = (
  ...
  wire(
    new RectElement(
      options,
      resolve_element_style(options.style, undefined, "container", theme),
    ),
    theme,
    parse_markdown,
  );

export const make_image = (
  options: CreateImageOptions,
  theme: Theme | null,
  parse_markdown: boolean,
): ImageElement => wire(new ImageElement(options), theme, parse_markdown);

export const make_container = (
  ...
  wire(
    new ContainerElement(
      options ?? {},
      resolve_element_style(options?.style, options?.classname, "container", theme),
    ),
    theme,
    parse_markdown,
  );
