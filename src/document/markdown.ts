...
type VariantResolver = (base: string, bold: boolean, italic: boolean) => string;
...
export function render_segments(
  pdoc: any,
  ...
  const pick: VariantResolver = variant ?? get_variant_font;
  ...
  if (has_fractions) {
    ...
    for (const seg of segments) {
      ...
      const font = seg.font ?? pick(base_font, seg.bold, seg.italic);
  ...
  const needs_formatting = segments.some(
    (s) =>
      s.bold || s.italic || s.strikethrough || s.link || s.color || s.font,
  );
  ...
  if (!needs_formatting) {
    ...
    pdoc.font(base_font).fontSize(font_size);
    if (position) {
      pdoc.text(plain, position.x, position.y, text_opts);
    } else {
      pdoc.text(plain, text_opts);
    }
    return;
  }
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    pdoc.font(font).fontSize(font_size);
    if (seg.color) pdoc.fillColor(seg.color);
    ...
    if (seg.color && base_color) pdoc.fillColor(base_color);
  }
